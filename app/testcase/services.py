...
class Service():
	...
	def forward_service(self,localport):
		forward(self.serial,self.ports[0],'stfservice')
		# forward_tcp(self.serial,localport,1100)
	def forward_agent(self,localport):
		forward(self.serial,self.ports[1],'stfagent')
		# forward_tcp(self.serial,localport,1090)
	def killProc(self,commn):
		res=kill(self.serial,commn)
	def killPid(self,pid):
		res=killPid(self.serial,pid)
	def install_service_apk(self):
		# uninstall(self.serial,self.service_resource['pkg'])
		print ('uninstall stfservice')
		# install(self.serial,self.service_resource['apk'])
	def installAll(self):
		self.killProc('stf.agent')
...
class stfServices():
	...
	def errorhandler(self,error='',location=''):
		...
		try:
			socketio.emit('system',{'name':'serviceStatus','data':error},namespace=self.namespace)
		except:
	...
	def __connectService(self):
		try:
			...
			if self.get_status==0 and self.send_status==0:
				...
				raise ServiceError('get:%s/send:%s Error'%(self.get_status,self.send_status),'__connectService')
		except Exception as e:
	...
	def _stop(self):
		...
		socketio.emit('system',{'name':'serviceStatus','data':'disconnected'},namespace=self.namespace)
		...
		if self.runningState==2 or self.runningState==4:
			...
			if self.desiredState.get()==3:
				try:
					...
					socketio.emit('system',{'name':'serviceStatus','data':'connected'},namespace=self.namespace)
				except Exception as e:
	...
	def notify(self,eventname,data):
		socketio.emit('event',{'eventname':eventname,'data':data},namespace=self.namespace)
